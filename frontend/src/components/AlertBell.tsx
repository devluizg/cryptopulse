...
// Cores padrão para severidades desconhecidas
...
function AlertDropdownItem({ alert, onClick }: { alert: AlertWithAsset; onClick: () => void }) {
  // Usa cores padrão se a severidade não existir no mapa
  const colors = ALERT_SEVERITY_COLORS[alert.severity] || DEFAULT_SEVERITY_COLORS;
  ...
  return (
    <button onClick={onClick} className={cn('w-full p-4 text-left transition-colors hover:bg-crypto-card/50', !alert.is_read && 'bg-crypto-card/30')}>
      <div className="flex items-start gap-3">
        <div className={cn('flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center text-lg', colors.bg)}>
          {cryptoIcon}
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className={cn('font-medium text-sm', colors.text)}>{alert.title}</span>
            {!alert.is_read && <span className="w-2 h-2 bg-score-high rounded-full flex-shrink-0" />}
          </div>
          <p className="text-xs text-crypto-muted line-clamp-2">{alert.message}</p>
          <span className="text-xs text-crypto-muted/70 mt-1 block">{formatRelativeTime(alert.created_at)}</span>
        </div>
      </div>
    </button>
  );
